...
pub async fn get_new_refresh_token() -> Result<(String)> {
    ...
    let login_url = format!("{}&code_challenge_method=S256&code_challenge={}", LOGIN_URL, challenge);
    ...
    let callback = warp::get()
        ...
        .and_then(move|p:CallbackParameters|{
            ...
            async move {
                // let code = p.code;
                info!("callback: {:?}", p);
                let token = exchange_code_for_token(p.code.as_str(), verifier_str.as_str()).await.expect("todo anyhow <-> warp");
                info!("token: {:?}", token);
    ...
    let test = warp::path::end() // matches /
        .map(move ||warp::redirect::found(Uri::from_str(login_url.as_str()).expect("should work")));

    info!("Please visit http://localhost:8080/");
    let (_addr, server) = warp::serve(test.or(callback))
