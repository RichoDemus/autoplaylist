...
pub async  fn read_file(filename: String) -> Option<Vec<u8>> {
 ...
 pub async fn write_file(filename: String, bytes:Vec<u8>) -> Result<()> {
    ...
    let mut file = File::create(format!("data/{}", filename)).context("Failed to open event disk file handler")?;

    file.write_all(bytes.as_slice()).context("Failed to write event to disk")?;
    Ok(())
    }).await?
}
