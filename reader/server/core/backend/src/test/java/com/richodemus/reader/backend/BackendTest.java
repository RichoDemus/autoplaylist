...
public class BackendTest {
    ...
    public void setUp() {
        ...
        target = new Backend(subscriptionRepository, feedRepository, userRepository);
    ...
    public void getFeedShouldReturnFeed() {
        final List<Item> unwatchedItems = FEED_1.getItems()
                .stream()
                .filter(i -> !EXISTING_USER.isRead(FEED_1.getId(), i.getId()))
                .map(i -> new Item(i.getId(), i.getTitle(), i.getDescription(), i.getUploadDate(), "https://youtube.com/watch?v=" + i.getId(), i.getDuration(), i.getViews()))
                .collect(toList());
        final Feed expected = new Feed(FEED_1.getId(), FEED_1.getName(), unwatchedItems);
        final Optional<Feed> result = target.getFeed(EXISTING_USER.getName(), FEED_1.getId());
