...
class UserService(
    ...
    @Synchronized
    fun findOrCreateUser(userId: UserId): User {
        val matchingUsers = users.values.filter { it.userId == userId }
        if (matchingUsers.isNotEmpty()) {
            if (matchingUsers.size > 1) {
                logger.warn("There seems to be more than 1 user stored with the id {}", userId)
            }
            if (matchingUsers.size == 1) {
                logger.info("User with id {} already exists", userId)
        ...
        val event = UserCreated(userId = userId)
        val user = add(event)
...
//    }
    ...
    private fun add(userCreated: UserCreated): User {
        ...
        val user = User(eventStore, spotifyPort, userCreated.userId)
